...
const MOCK_LOGS: Record<string, any[]> = {
  'default': [
    { 
      title: '波段合成', 
      time: '2026-02-02 01:01:32', 
      status: 'success', 
      statusText: '执行成功',
      detail: '合成 4-3-2 (RGB) 真彩色影像。',
      result: '生成 TIF 成果文件：/data/output/v2/merged_B432.tif，大小 3.5GB。',
      extra: '元数据校验通过：波段完整性 100%，坐标参考系 EPSG:4326。'
    },
    { 
      title: '数据采集', 
      time: '2026-02-02 01:00:09', 
      status: 'success', 
      statusText: '执行成功',
      detail: '从欧空局 Sentinel-2 归档库获取数据。',
      result: '成功拉取 12 个切片，总大小 15.6GB，已存入临时缓存区。'
    },
    { 
      title: '影像镶嵌', 
      time: '2026-02-02 01:02:34', 
      status: 'success', 
      statusText: '执行成功',
      detail: '执行多幅影像重叠度分析及缝合线生成。',
      result: '镶嵌精度符合要求（RMS < 0.5px）。',
      extra: '输出路径：/data/output/temp_mosaic.tif'
    },
    { 
      title: '影像裁剪', 
      time: '2026-02-02 01:03:14', 
      status: 'success', 
      statusText: '执行成功',
      detail: '根据 ROI 矢量范围（武汉市行政边界）进行硬裁剪。',
      result: '产出目标子场景影像 1 幅，已剔除无值区域（NoData）。'
    },
    { 
      title: '耕地范围提取', 
      time: '2026-02-02 09:03:32', 
      status: 'pending', 
      statusText: '等待执行',
      detail: '等待前序 [影像裁剪] 节点释放文件锁...' 
    }
  ],
  'failed': [
    { 
      title: '波段合成', 
      time: '2026-01-14 18:03:06', 
      status: 'success', 
      statusText: '执行成功',
      detail: '完成原始波段配准。'
    },
    ...
    { 
      title: '影像镶嵌', 
      time: '2026-01-14 18:04:00', 
      status: 'error', 
      statusText: '执行异常',
      errorType: '资源限制 (OOM)',
      errorCode: 'ERR_WORKER_OUT_OF_MEMORY',
      detail: '算法在处理 10GB 以上超大规模 TIF 镶嵌时，物理内存 (64GB) 耗尽。',
      reason: '镶嵌缓存分块大小 (TileSize) 设置过大（10240px），导致算子溢出。',
      suggestion: '请尝试在任务配置中将 “单片分块大小” 调小至 2048px，或增加算子节点内存配额。'
    },
    { 
      title: '影像裁剪', 
      time: '-', 
      status: 'skipped', 
      statusText: '跳过执行',
      detail: '由于前置环节 [影像镶嵌] 失败，根据“失败策略：停止”，本环节及后续环节已自动跳过。' 
    }
  ]
};
...
/* Add Helper Components for Task Progress View */
const NodeComponent = ({ icon, label, status, progress }: { icon: React.ReactNode; label: string; status: NodeStatus; progress?: number }) => {
  return (
  ...
  return (
    <div className="flex-1 flex flex-col bg-white/40 backdrop-blur-xl rounded-2xl shadow-2xl shadow-blue-900/10 border border-white/50 p-4 gap-4 overflow-hidden relative h-full font-['Noto_Sans_SC']">
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-[800px] rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in slide-in-from-top-4 duration-300">
            <div className="px-8 py-6 flex items-center justify-between bg-white border-b border-slate-100">
              ...
              <button onClick={() => setIsConfigModalOpen(false)} className="text-slate-400 hover:text-slate-600 transition-all hover:rotate-90">
                <X size={24} />
              </button>
            </div>
            <div className="p-10 space-y-10 overflow-y-auto max-h-[75vh] custom-scrollbar">
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[14px] font-bold text-slate-700 flex items-center gap-1">
                      <span className="text-rose-500">*</span> 任务名称
                    </label>
                    <input 
                      type="text" 
                      placeholder="请输入任务名称"
                      className="w-full h-12 px-4 bg-white border border-slate-200 rounded-xl text-[14px] text-slate-700 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm"
                  ...
                  <div className="space-y-2">
                    <label className="text-[14px] font-bold text-slate-700 flex items-center gap-1">
                      <span className="text-rose-500">*</span> 输出路径
                    </label>
                    <div className="flex gap-3">
                      <input 
                        type="text" 
                        placeholder="[任务]_[20260128103823]_202601281736"
                        className="flex-1 h-12 px-4 bg-white border border-slate-200 rounded-xl text-[14px] text-slate-600 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm"
                        value={taskConfig.outputPath}
                        onChange={(e) => setTaskConfig({...taskConfig, outputPath: e.target.value})}
                      />
                      <button 
                        onClick={() => setIsOutputSelectorOpen(true)}
                        className="px-6 h-12 bg-blue-50 text-blue-600 border border-blue-100 rounded-xl font-bold text-[14px] hover:bg-blue-600 hover:text-white transition-all shadow-sm active:scale-95 whitespace-nowrap"
                      >
                        浏览
                      </button>
                    </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                   <div className="w-1.5 h-6 bg-blue-600 rounded-full shadow-md shadow-blue-500/20"></div>
                   <h4 className="text-[16px] font-bold text-slate-800">调度配置</h4>
                </div>
                <div className="space-y-8">
                  <div className="space-y-3">
                    <label className="text-[14px] font-bold text-slate-700 flex items-center gap-1">
                      <span className="text-rose-500">*</span> 任务类型
                    </label>
                    <div className="flex gap-12">
                      <label className="flex items-center gap-3 cursor-pointer group">
                        <input 
                          type="radio" 
                          checked={taskConfig.taskType === 'once'}
                          onChange={() => setTaskConfig({...taskConfig, taskType: 'once'})}
                          className="hidden"
                        />
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${taskConfig.taskType === 'once' ? 'border-blue-600 ring-4 ring-blue-50' : 'border-slate-300'}`}>
                          {taskConfig.taskType === 'once' && <div className="w-2.5 h-2.5 rounded-full bg-blue-600 shadow-sm" />}
                        </div>
                      ...
                      <label className="flex items-center gap-3 cursor-pointer group">
                        <input 
                          type="radio" 
                          checked={taskConfig.taskType === 'timer'}
                          onChange={() => setTaskConfig({...taskConfig, taskType: 'timer'})}
                          className="hidden"
                        />
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${taskConfig.taskType === 'timer' ? 'border-blue-600 ring-4 ring-blue-50' : 'border-slate-300'}`}>
                          {taskConfig.taskType === 'timer' && <div className="w-2.5 h-2.5 rounded-full bg-blue-600 shadow-sm" />}
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-10">
                    <div className="space-y-2">
                      <label className="text-[14px] font-bold text-slate-700 flex items-center gap-1">
                        <span className="text-rose-500">*</span> 失败策略
                      </label>
                      <div className="relative group">
                        <select 
                          className="w-full h-12 px-4 appearance-none bg-white border border-slate-200 rounded-xl text-[14px] text-slate-700 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all cursor-pointer shadow-sm"
                          value={taskConfig.errorStrategy}
                          onChange={(e) => setTaskConfig({...taskConfig, errorStrategy: e.target.value})}
                        >
                    ...
                    <div className="space-y-2">
                      <label className="text-[14px] font-bold text-slate-700 flex items-center gap-1">
                        <span className="text-rose-500">*</span> 报警方式
                      </label>
                      <div className="relative group">
                        <select 
                          className="w-full h-12 px-4 appearance-none bg-white border border-slate-200 rounded-xl text-[14px] text-slate-700 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all cursor-pointer shadow-sm"
                          value={taskConfig.alarmType}
                          onChange={(e) => setTaskConfig({...taskConfig, alarmType: e.target.value})}
                        >
            ...
            <div className="px-10 py-8 bg-slate-50/50 border-t border-slate-100 flex justify-center gap-6 shrink-0">
              <button 
                ...
                className="w-40 h-12 rounded-xl border-2 border-blue-600 text-blue-600 font-black text-[15px] hover:bg-blue-50 transition-all shadow-sm active:scale-95"
              >
                取消
              </button>
              <button 
                onClick={handleSaveConfigAndOrchestrate}
                className="w-40 h-12 rounded-xl bg-blue-600 text-white font-black text-[15px] hover:bg-blue-700 shadow-xl shadow-blue-500/30 transition-all active:scale-95"
              >
                保存配置
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-[800px] rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in slide-in-from-top-4 duration-300">
            <div className="px-8 py-6 flex items-center gap-3 bg-white border-b border-slate-100">
              ...
              <button onClick={() => setIsEditModalOpen(false)} className="ml-auto text-slate-400 hover:text-slate-600 transition-all hover:rotate-90">
                <X size={24} />
              </button>
            </div>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full h-12 px-4 bg-white border border-slate-200 rounded-xl text-[14px] text-slate-800 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm font-bold"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-3">
                      <input 
                        type="text" 
                        value={editingTask.outputPath}
                        onChange={(e) => setEditingTask({...editingTask, outputPath: e.target.value})}
                        className="flex-1 h-12 px-4 bg-white border border-slate-200 rounded-xl text-[14px] text-slate-600 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm"
                      />
                      <button 
                        onClick={() => setIsOutputSelectorOpen(true)}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-12">
                      <label className="flex items-center gap-3 cursor-pointer group">
                        <input 
                          type="radio" 
                          checked={editingTask.type === '一次性任务'}
                          onChange={() => setEditingTask({...editingTask, type: '一次性任务'})}
                          className="hidden"
                        />
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${editingTask.type === '一次性任务' ? 'border-blue-600 ring-4 ring-blue-50' : 'border-slate-300'}`}>
                          {editingTask.type === '一次性任务' && <div className="w-2.5 h-2.5 rounded-full bg-blue-600 shadow-sm" />}
                        </div>
                        <span className={`text-[14px] font-bold ${editingTask.type === '一次性任务' ? 'text-blue-600' : 'text-slate-600'}`}>一次性任务</span>
                      </label>
                      <label className="flex items-center gap-3 cursor-pointer group">
                        <input 
                          type="radio" 
                          checked={editingTask.type === '定时任务'}
                          onChange={() => setEditingTask({...editingTask, type: '定时任务'})}
                          className="hidden"
                        />
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${editingTask.type === '定时任务' ? 'border-blue-600 ring-4 ring-blue-50' : 'border-slate-300'}`}>
                          {editingTask.type === '定时任务' && <div className="w-2.5 h-2.5 rounded-full bg-blue-600 shadow-sm" />}
                        </div>
                        <span className={`text-[14px] font-bold ${editingTask.type === '定时任务' ? 'text-blue-600' : 'text-slate-600'}`}>定时任务</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-10">
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <select 
                          className="w-full h-12 px-4 appearance-none bg-white border border-slate-200 rounded-xl text-[14px] text-slate-700 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all cursor-pointer shadow-sm"
                          value={editingTask.errorStrategy}
                          onChange={(e) => setEditingTask({...editingTask, errorStrategy: e.target.value})}
                        >
                          <option value="stop">停止</option>
                          <option value="ignore">忽略并继续</option>
                        </select>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <select 
                          className="w-full h-12 px-4 appearance-none bg-white border border-slate-200 rounded-xl text-[14px] text-slate-700 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all cursor-pointer shadow-sm"
                          value={editingTask.alarmType}
                          onChange={(e) => setEditingTask({...editingTask, alarmType: e.target.value})}
                        >
                          <option value="system">系统通知</option>
                          <option value="email">邮件通知</option>
                          <option value="sms">短信通知</option>
            ...
            <div className="px-10 py-8 bg-slate-50/50 border-t border-slate-100 flex justify-center gap-6 shrink-0">
              <button 
                ...
                className="w-40 h-12 rounded-xl border-2 border-blue-600 text-blue-600 font-black text-[15px] hover:bg-blue-50 transition-all active:scale-95 shadow-sm"
              >
                取消修改
              </button>
              <button 
      ...
      {isTerminateModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-[420px] rounded-2xl shadow-2xl p-8 relative animate-in zoom-in duration-300">
             <button onClick={() => setIsTerminateModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
               <X size={20} />
             </button>
             <div className="flex flex-col items-center text-center">
               ...
               <p className="text-slate-500 text-[14px] mb-8 font-medium leading-relaxed">
                 您确认要终止当前运行任务？<br/>终止后该任务实例将立即停止。
               </p>
      ...
      {isRestartModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-[420px] rounded-2xl shadow-2xl p-8 relative animate-in zoom-in duration-300">
             <button onClick={() => setIsRestartModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
               <X size={20} />
             </button>
             <div className="flex flex-col items-center text-center">
               <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mb-6">
                  <RotateCcw size={40} className="text-blue-600" strokeWidth={2.5} />
               </div>
               <h3 className="text-[18px] font-bold text-slate-800 mb-2">重启任务确认</h3>
               <p className="text-slate-500 text-[14px] mb-8 font-medium leading-relaxed">
                 您确定要重启该任务吗？<br/>重启后，任务将重置并从起始节点开始执行。
               </p>
               <div className="flex w-full gap-4">
      ...
      {isLogModalOpen && taskForLog && (
        <div className="fixed inset-0 z-[250] flex justify-end">
          ...
          <div className="relative w-[520px] h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
               <div className="flex items-center gap-3">
                 <div className="p-2 bg-blue-600 rounded-lg text-white shadow-lg shadow-blue-500/20"><Terminal size={22} /></div>
                 <div>
                    <h3 className="text-[18px] font-black text-slate-800 tracking-tight">任务执行日志</h3>
                    <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Runtime Logs & Diagnostics</p>
                 </div>
               </div>
               <button onClick={() => setIsLogModalOpen(false)} className="text-slate-400 hover:text-slate-600 transition-all hover:rotate-90">
                 <X size={24} />
               </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 bg-slate-50/30">
              {(MOCK_LOGS[taskForLog.status === '失败' ? 'failed' : 'default']).map((log, idx) => (
                <div key={idx} className="relative pl-6 animate-in slide-in-from-right-4" style={{ animationDelay: `${idx * 100}ms` }}>
                  {/* 垂直连接线 */}
                  {idx < (MOCK_LOGS[taskForLog.status === '失败' ? 'failed' : 'default']).length - 1 && (
                    <div className="absolute left-[3px] top-4 bottom-[-24px] w-0.5 bg-slate-200/60" />
                  )}
                  {/* 指示圆点 */}
                  <div className={`absolute left-0 top-1.5 w-2 h-2 rounded-full border-2 bg-white z-10 ${
                    log.status === 'success' ? 'border-emerald-500 ring-4 ring-emerald-50' : 
                    log.status === 'error' ? 'border-rose-500 ring-4 ring-rose-50' : 
                    log.status === 'pending' ? 'border-blue-500 ring-4 ring-blue-50' : 'border-slate-300'
                  }`} />

                  <div className={`bg-white border rounded-2xl overflow-hidden shadow-sm transition-all hover:shadow-md ${
                    log.status === 'error' ? 'border-rose-100 ring-1 ring-rose-50' : 'border-slate-100'
                  }`}>
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <h4 className="text-[15px] font-black text-slate-800 flex items-center gap-2">
                                {log.title}
                                <span className={`text-[10px] px-1.5 py-0.5 rounded font-black uppercase tracking-tighter ${
                                    log.status === 'success' ? 'bg-emerald-50 text-emerald-600' :
                                    log.status === 'error' ? 'bg-rose-50 text-rose-600' : 'bg-slate-100 text-slate-500'
                                }`}>
                                    {log.statusText}
                                </span>
                            </h4>
                            <span className="text-[10px] font-mono font-bold text-slate-400 mt-1">{log.time}</span>
                        </div>
                      </div>

                      <div className="space-y-3">
                        <div className="text-[13px] text-slate-500 font-medium leading-relaxed bg-slate-50/50 p-3 rounded-xl border border-slate-100/50">
                            {log.detail}
                        </div>

                        {log.result && (
                            <div className="flex items-start gap-2.5 text-[13px] text-blue-700 font-bold bg-blue-50/50 p-3 rounded-xl border border-blue-100/50">
                                <FileSearch size={16} className="shrink-0 mt-0.5 text-blue-500" />
                                <span>{log.result}</span>
                            </div>
                        )}

                        {log.extra && (
                            <div className="flex items-start gap-2.5 text-[12px] text-emerald-700 font-bold bg-emerald-50/50 p-3 rounded-xl border border-emerald-100/50">
                        ...
                        {log.status === 'error' && (
                            <div className="space-y-3 pt-1">
                                <div className="flex items-start gap-2.5 text-[13px] text-rose-700 font-black bg-rose-50 p-3 rounded-xl border border-rose-200 shadow-sm animate-pulse">
                                    <ZapOff size={16} className="shrink-0 mt-0.5" />
                                    <div>
                                        <p className="text-[11px] uppercase tracking-widest opacity-60 mb-1">Diagnostic Report / {log.errorType}</p>
                                        {log.reason}
                                    </div>
                                </div>
                                <div className="flex items-start gap-2.5 text-[12px] text-slate-600 font-bold bg-white p-3 rounded-xl border-2 border-dashed border-slate-200">
                                    <Settings size={16} className="shrink-0 mt-0.5 text-slate-400" />
                                    <div>
                                        <span className="text-slate-400 mr-1">建议解决方案:</span>
                                        {log.suggestion}
                                    </div>
                                </div>
                            </div>
            ...
            <div className="p-8 bg-white border-t border-slate-100 shrink-0">
               <button className="w-full h-13 bg-slate-900 text-white rounded-2xl font-black text-[15px] flex items-center justify-center gap-3 hover:bg-black shadow-2xl shadow-slate-900/20 transition-all active:scale-95">
                 <Download size={20} /> 下载完整运行报告 (.log)
               </button>
            </div>
          </div>
        </div>
        ...
        onConfirm={(path) => {
            if (typeof path === 'string') {
                if (isEditModalOpen && editingTask) {
                    ...
                    setTaskConfig({...taskConfig, outputPath: path});
                }
            }
